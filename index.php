...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        // Dapatkan id_organisasi yang sesuai dengan id_akun
    $sql_org = "SELECT id_organisasi FROM organisasi WHERE id_akun = ?";
    $stmt_org = $conn->prepare($sql_org);
    $stmt_org->bind_param("i", $row['id_akun']);
    $stmt_org->execute();
    $result_org = $stmt_org->get_result();
    
    if ($result_org->num_rows > 0) {
        $row_org = $result_org->fetch_assoc();
        $_SESSION['id_organisasi'] = $row_org['id_organisasi']; // Simpan id_organisasi di sesi
    }

        if ($row['role'] == 'penilai') {
            header("Location: penilai/penilai_beranda.php"); // Path ke dashboard penilai
        } elseif ($row['role'] == 'user') { // Ganti 'upt' menjadi 'user'
            header("Location: upt/index.php"); // Path ke user dashboard
        }
        exit();
        ...
        <div class="container text-center" style="margin-top:30px;">
            <div class="row">
                ...
                <div class="col login">
                    ...
                    <a class="dropdown-item pb-4" href="upt/index.php" id="bor">Kembali ke Beranda</a>
                </div>
                <div class="col"></div>
            </div>
        </div>
